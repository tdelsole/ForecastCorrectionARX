traditional.correction = function(f.ts,f.date,o.ts,o.date,nlead,nstart,nens,train.str=NA,train.end=NA) {
...
for (mon in 1:12) for (lead in 1:nlead) {
	...
	forecast.anom[lead,npic] = f.ts[lead,npic] - mean(f.ts[lead,npic.train])
}
...
for (mon in 1:12) {
	...
	obs.anom[npic] = o.ts[npic] - mean(o.ts[npic.train])
	obs.clim[npic] = mean(o.ts[npic.train])
}
