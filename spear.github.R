...
################################################
################################################
# file.forcing  = '/Users/delsole/Documents/Data/CMIP6_Forcing/CMIP6.forcing.Annex.RData'
file.forcing  = paste(dir.forc,'CMIP6.forcing.Annex.RData',sep='')
load(file.forcing)
...
###################################################################
plot.str     = as.Date('15-01-1970',format='%d-%m-%Y')
...
for (nf in 1:nforcing) {
	...
	if (forcing.say[nf] == 'AER') col.pic = c(col.pic,'red')
	if (forcing.say[nf] == 'NAT') col.pic = c(col.pic,'blue')
...
title(ftitle.top,line=2.0)
title(ftitle.bot,line=0.5)
legend('bottomright',legend=case.say,pch=19,col=1:ncases,cex=1.5,bg='white')
if (lplotfile) dev.off()
...
for (mpic in c(1,4,7,10)) {
	fout = paste(dir.figs,'SPEAR.',model.forcing,'.lead',lead.pic,'error.',month.abb[mpic],'.traditional',sep='')
	if (lplotfile) pdf.eps(fout,'pdf',height=5)
	par(mfcol=c(1,1))
	par(mar=c(8,5,3,1))
	par(cex.lab=1.3,cex.axis=1.3,cex.main=1.3)
	nget = which( month(spear.list$targ.nmme[1,,1]) == mpic )
	plot  (spear.list$targ.nmme[lead.pic,nget,],err.nmme.old[lead.pic,nget,],cex=0.5,xlab='',ylab='error',col='blue',ylim=yrange,pch=19)
	abline(h=0,col='grey30',lty='dashed',lwd=2)
	ftitle.top = paste('Error After Traditional Bias Correction')
	ftitle.bot = paste("Lead: ",lead.pic-0.5,'; Target: ',month.abb[mpic],sep='')
	title(ftitle.top,line=2.0)
	title(ftitle.bot,line=0.5)
	#legend('topleft',legend=c('traditional bias correction','ARX bias correction'),pch=c(1,4),col=c('blue','red'),cex=1.2)
	if (lplotfile) dev.off()
...
for (mpic in c(1,4,7,10)) {
	fout = paste(dir.figs,'SPEAR.',model.forcing,'.lead',lead.pic,'error.',month.abb[mpic],'.ARXcorrection',sep='')
	if (lplotfile) pdf.eps(fout,'pdf',height=5)
	par(mfcol=c(1,1))
	par(mar=c(8,5,3,1))
	par(cex.lab=1.3,cex.axis=1.3,cex.main=1.3)
	...
	plot  (spear.list$targ.nmme[lead.pic,nget,],err.nmme.arx[lead.pic,nget,],cex=0.5,xlab='',ylab='error',col='red' ,ylim=yrange,pch=19)
	abline(h=0,col='grey',lty='dashed',lwd=2)
	ftitle.top = paste('Error After ARX Bias Correction')
	ftitle.bot = paste("Lead: ",lead.pic-0.5,'; Target: ',month.abb[mpic],'; ARX Fit: H= ',nharm,'; P=',p.order,'; ',region,sep='')
	title(ftitle.top,line=2.0)
	title(ftitle.bot,line=0.5)
	#legend('topleft',legend=c('traditional bias correction','ARX bias correction'),pch=c(1,4),col=c('blue','red'),cex=1.2)
	if (lplotfile) dev.off()
...
ftitle.top = paste("Root Mean Square Errors")
ftitle.bot = paste('ARX Fit: H= ',nharm,'; P=',p.order,'; ',region,sep='')
title(ftitle.top,line=2.0)
title(ftitle.bot,line=0.5)
if (lplotfile) dev.off()
...
if (lplotfile) pdf.eps(fout,'pdf')
yrange = sqrt(range(mse.nmme.arx.train[-(1:p.order)],mse.nmme.old.train,mse.nmme.arx.valid[-(1:p.order)],mse.nmme.old.valid,mse.best.arx.valid[-(1:p.order)]))
xrange = c(1,nlead.nmme)-0.5
par(mfcol=c(1,1),mar=c(8,5,3,1))
par(cex.lab=1.3,cex.axis=1.3,cex.main=1.3)
plot(1,1,type='n',xlim=xrange,ylim=yrange,xlab='lead (months)',ylab='Root Mean Square Error (degrees C)',xaxt='n')
axis(1,1:nlead.nmme-0.5,labels=1:nlead.nmme-0.5)
lead.clip = (1+p.order):nlead.nmme
# lines(1:nlead.nmme-0.5,sqrt(mse.nmme.old.train),col='black',lwd=2,lty='dashed')
# lines(lead.clip-0.5,sqrt(mse.best.arx.train[lead.clip]),col='blue',lwd=2,lty='dashed')
# lines(lead.clip-0.5,sqrt(mse.nmme.arx.train[lead.clip]),col='red',lwd=2,lty='dashed')
...
########## PLOT LEAD-1 ERRORS: TIME SERIES
##################################################################
fout = paste(dir.figs,'SPEAR.',model.forcing,'.lead1error.kf',suffix.forcing,sep='')
if (lplotfile) pdf.eps(fout,'pdf')
par(mfcol=c(1,1),mar=c(5,5,3,1))
par(cex.lab=1.3,cex.axis=1.3,cex.main=1.3)
